...
static inline void volk_gnsssdr_16ic_xn_resampler_16ic_xn_rvv(lv_16sc_t** result, const lv_16sc_t* local_code, float rem_code_phase_chips, float code_phase_step_chips, float* shifts_chips, unsigned int code_length_chips, int num_out_vectors, unsigned int num_points)
{
    ...
    for (int current_correlator_tap = 0; current_correlator_tap < num_out_vectors; current_correlator_tap++)
        {
            // Stores indices from `local_code` to load from and
            ...
            for (size_t vl; n > 0; n -= vl, outPtr += vl, overflowIndexPtr += vl)
                {
                    ...
                    vuint32m8_t overflowIndexVal = __riscv_vle32_v_u32m8(overflowIndexPtr, vl);

                    // Wrap to valid index in `local_code`
                    // index[i] = overflowIndex[i] % code_length_chips
                    vuint32m8_t indexVal = __riscv_vremu_vx_u32m8(overflowIndexVal, code_length_chips, vl);

                    // Convert to address offset
                    // offset[i] = index[i] * sizeof(lv_16_sc_t)
                    vuint32m8_t offsetVal = __riscv_vmul_vx_u32m8(indexVal, sizeof(lv_16sc_t), vl);
